...
2. Add this appsettings.json configuration block and provide appropriate values. Such as, for production, collect and set production information and set "ProductionMode" value as true. And for sandbox/testing mode, similarly set sandbox/testing information and set "ProductionMode" to false.
    ```
    "BkashOptions": {
        "MerchantUserName": "",
        "MerchantPassword": "",
    ...
    ```
3. Create a class named "BkashOptions" to inject the configuration values from AppSettings. As it is a good practice.
    ```
    ...
    ```
4. Now, add dependency calling AddBkash inside IOC container:
    ```
    // Bind AppSettings configuration information from "BkashOptions".
    var bkashOptions = new BkashOptions();
    builder.Configuration.GetSection("BkashOptions").Bind(bkashOptions);

    // Add bkash dependency providing values.
...
Payment-creation is one of the parts of the complete payment flow. In this process,
1. We create a payment instance against a "UNIQUE_INVOICE_NUMBER".
...
5. User will provide his Bkash credentials (ie. number, pin, OTP etc).
6. As soon as the user completes these tasks, he will be redirected to the callback URL(___"https://example_server_or_client.com/api/wallets/bkash/callback"___) we provided below.
7. This ___CallBack___ URL is basically our Server or Client's URL ___(if its a server URL then it should be a get method)___, where we want to receive user payment creation status is OK or not, so that we can proceed to [Payment Execution](#payment-execution-process).
